...
def build_postgres_dsn() -> str:
    # SD_DB_HOST tem prioridade — Supabase ou outro banco externo
    ...
    if host:
        ...
        # Usa URL format para evitar problemas com hostaddr e caracteres especiais na senha
        from urllib.parse import quote_plus
        pw = quote_plus(password) if password else ""
        auth = f"{user}:{pw}@" if pw else f"{user}@"
        return f"postgresql://{auth}{host}:{port}/{dbname}?sslmode=require"

    # Fallback: DATABASE_URL do Railway
